...
NodeBullet::NodeBullet(Vector2 alpha, Vector2 beta) : Entity2D("NodeBullet", alpha) {
    targetLifetime = Vector2Distance(alpha, beta) / speed;
...
void NodeBullet::process(float delta) {
    lifetime += delta;
    position += (Vector2){delta * speed * cos(theta), delta * speed * sin(theta)};
    if(lifetime >= targetLifetime) killDefered();
