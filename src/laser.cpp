...
void Laser::render() {
    ...
    if(preWrap.x == endPos.x && preWrap.y == endPos.y)
        ...
        while(abs(preWrap.x - endPos.x) > 10 && abs(preWrap.y - endPos.y) > 10) {
            // get intersection
            ...
            float yPosAtBorder =
              rayOrigin.y + (slope * ((left ? Border::length : -Border::length) - rayOrigin.x));
            float xPosAtBorder =
              rayOrigin.x + (1 / slope) * ((top ? Border::length : -Border::length) - rayOrigin.y);
            bool swapX = abs(yPosAtBorder) <= Border::length;
            bool swapY = abs(xPosAtBorder) <= Border::length;
            // so the gist with this variable is if we collide on x we set it to be the y it
            // collides on and vise versa
            if(swapX) {
            ...
            rayOrigin = {
              swapX ? -collisionPosition.x : collisionPosition.x,
              !swapX ? -collisionPosition.y : collisionPosition.y
            };
