...
HealthPack::HealthPack(Vector2 p, float h)
  : Entity2D("HealthPack", p), health(h), velCart({0, 0}), velPol(0), isAttracted(false) {}
HealthPack::HealthPack(Vector2 p, Vector2 v, float h)
  : Entity2D("HealthPack", p), health(h), velCart(v), velPol(0), isAttracted(false) {}
...
bool HealthPack::getIsAttracted() {
    return isAttracted;
}
...
void HealthPack::process(float delta) {
    ...
    // the latter however is for when we're picked up, so that way it's a generally consistent time
    // to be picked up (also it looks really cool)
    ...
    if(!isAttracted) {
        ...
        if(Vector2Length(offset) <= cutoffDistance) killDefered();
    }
}
