...
// since it's a taurus there's some... funky geometry... (Shortest paths are still shortest paths,
// just with wrapping applied) it's a static class* *(exception being rendering) and so most of the
// functions and values in here are static.
