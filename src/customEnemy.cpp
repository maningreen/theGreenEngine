...
void CustomEnemy::manageStates(float delta) {
    if(manageStateCustom.valid()) manageStateCustom(this, delta);
}

void CustomEnemy::onDeath() {
    if(onDeathCustom.valid()) onDeathCustom(this);
}

void CustomEnemy::dropHealth() {
    if(dropHealthCustom.valid()) dropHealthCustom(this);
}

void CustomEnemy::onSpawn() {
    if(onSpawnCustom.valid()) onSpawnCustom(this);
}
...
std::optional<CustomEnemy> CustomEnemy::fromTable(sol::table x) {
    ...
    if(!(r && name && s && c && spawn && drop && state && death)) {
        if(!r.valid()) std::cout << "missing radius\n";
        if(!name.has_value()) std::cout << "missing name\n";
        if(!s.valid()) std::cout << "missing initialState\n";
        if(!c.valid()) std::cout << "missing color\n";
        if(!spawn.valid()) std::cout << "missing onSpawn\n";
        if(!drop.valid()) std::cout << "missing dropHealth\n";
        if(!state.valid()) std::cout << "missing manageState\n";
        if(!death.valid()) std::cout << "missing onDeath\n";
        return std::nullopt;
    } else
