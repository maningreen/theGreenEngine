...
// gur getters :p
// static
float Sniper::getMinDist() {
    return minDist;
}
float Sniper::getMaxDist() {
    return maxDist;
}
float Sniper::getRotationSpeed() {
    return rotationSpeed;
}
float Sniper::getSpeed() {
    return speed;
}
float Sniper::getDefaultHealth() {
    return defaultHealth;
}
float Sniper::getAimTime() {
    return aimTime;
}
float Sniper::getShotRecoil() {
    return shotRecoil;
}
float Sniper::getBulletSpeed() {
    return bulletSpeed;
}
float Sniper::getWidthGrowthRate() {
    return widthGrowthRate;
}
float Sniper::getRadius() {
    return radius;
}
Color Sniper::getDefaultColour() {
    return defaultColour;
}
Color Sniper::getLaserColour() {
    return laserColour;
}
// regular
float Sniper::getRotation() const {
    return rotation;
}
float Sniper::getRotationalVelocity() const {
    return rotationalVelocity;
}
Vector2 Sniper::getTargetPosition() const {
...
void Sniper::manageStates(float delta) {
    ...
    if(getState() == positioning) {
        targetPosition = getClosestPointToPlayerWithDistance(
          std::clamp(Border::getDistance(position, Player::get().position), minDist, maxDist)
        );
        ...
        if(Vector2Distance(position, targetPosition) < maxDistFromTargetPos) setState(aiming);
    } else if(getState() == aiming) {
        if(las->length < minDist) setState(positioning);
        ...
        if(getStateTime() > aimTime) {
            ...
            velocity = Vector2Add(
              velocity,
              (Vector2){-cosf(rotation) * shotRecoil, -sinf(rotation) * shotRecoil}
            );
