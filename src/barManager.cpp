...
BarManager::BarManager(Vector2 Position, float targetDistance, Bar childBar)
  : Entity("BarManager"), bar(childBar), targetDistance(targetDistance), positionPointer(0) {
...
void BarManager::process(float delta) {
    ...
    bar.position = Vector2Subtract(
      Vector2Add(
        *positionPointer,
        !bar.ShrinkY ? (Vector2){0, targetDistance} : (Vector2){targetDistance, 0}
      ),
      !bar.ShrinkY ? (Vector2){bar.Dimensions.x / 2.0f, 0} : (Vector2){0, bar.Dimensions.y / 2.0f}
    );
