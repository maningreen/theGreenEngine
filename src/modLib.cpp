...
void ModManager::initLua() {
    ...
    sol::usertype<DashManager> dm = lua.new_usertype<DashManager>("dashManager");
    ...
    lua.new_usertype<keybind>("keybind", sol::constructors<keybind(Key, bool, sol::function)>());

    lua.new_usertype<keybindAlt>("keybind", sol::constructors<keybind(Key, bool, sol::function)>());

    sol::usertype<InputManager> im = lua.new_usertype<InputManager>("inputManager");
    im["addVectorBind"] = &InputManager::addVectorBind;
    im["addBind"] = sol::overload(
      [](InputManager* self, const keybind& b) {
          self->addBind(b);
      },
      [](InputManager* self, const keybindAlt& b) {
          self->addBind(b);
      }
    );
    ...
    sol::usertype<Enemy> en = lua.new_usertype<Enemy>("enemy", sol::constructors<Enemy(Vector2)>());

    sol::table global = lua["Global"].get_or_create<sol::table>();
    global.set_function("getFriction", []() {
        return Entity2D::friction;
    });
    global.set_function("setFriction", [](float f) {
        Entity2D::friction = f;
    });
    global.set_function("getPlayer", []() {
        return Player::get();
    });
    ...
    border["getShortestPathToPoint"] = [](const Vector2 a, const Vector2 b) {
    ...
    customEn["getClosestPointToPlayerWithDistance"] =
      &CustomEnemy::getClosestPointToPlayerWithDistance;
