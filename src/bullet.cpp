...
Bullet::Bullet(Vector2 position, float angle, bool w)
  : Entity2D("Bullet", position), Angle(angle), shouldWrap(w) {
    velocity = (Vector2){cosf(angle) * DefaultSpeed, -sinf(angle) * DefaultSpeed};
    col = DefaultColour;
    lifetime = DefaultMaxLifetime;
}

Bullet::Bullet(Vector2 position, float angle, float maxLifetime)
  : Entity2D("bullet", position), Angle(angle), lifetime(maxLifetime) {
    velocity = (Vector2){cosf(angle) * DefaultSpeed, -sinf(angle) * DefaultSpeed};
    col = DefaultColour;
}

Bullet::Bullet(Vector2 position, float angle, bool shouldWrap, float maxLifetime)
  : Entity2D("bullet", position), Angle(angle), shouldWrap(shouldWrap), lifetime(maxLifetime) {
    velocity = (Vector2){cosf(angle) * DefaultSpeed, -sinf(angle) * DefaultSpeed};
    col = DefaultColour;
}
...
void Bullet::process(float delta) {
    ...
    for(int i = 0; i < stepCount; i++) {
        ...
        if(shouldWrap) Border::wrapEntity(this);
    }
    lifetime -= delta;
    if(lifetime < 0) killDefered();
}
...
void Bullet::render() {
    ...
    DrawRectanglePro(
      (Rectangle){position.x, position.y, bulletDimensions.x, bulletDimensions.y},
      (Vector2){bulletDimensions.x / 2.0f, bulletDimensions.y / 2.0f},
      -Angle * 180 / M_PI,
      col
    );
}
