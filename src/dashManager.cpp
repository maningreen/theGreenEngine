...
int DashManager::getAvailableDashes() {
    return floorf(progress);
}

bool DashManager::canDash() {
    return progress >= 1 && !isDashing();
}

bool DashManager::isDashing() {
    return dashing;
}

float DashManager::getDashProgress() {
    return progress;
}

void DashManager::addDashProgress() {
    progress += 1;
    if(progress >= maxDashCount) progress = maxDashCount;
}
...
void DashManager::removeDashProgress() {
    if(getAvailableDashes()) --progress;
    deltaDash = 0;
}
