...
#define vec4(a, b, c, d)                                                                    \
    (Color) {                                                                               \
        (unsigned char)((a) * 255), (unsigned char)((b) * 255), (unsigned char)((c) * 255), \
          (unsigned char)((d) * 255)                                                        \
    }
...
Button::Button(Vector2 p, std::string t, std::function<void(Button*)> c)
  : Entity2D("Button", p), callback(c), text(t) {
...
void Button::process(float delta) {
    ...
    if(state != Passing) {
        ...
        relativeMousePos =
          (Vector2){
            ...
            relativeMousePos.y * angleVec.x +
              relativeMousePos.x * angleVec.y  // [ 0, sin(theta) ] i think
          } +
    ...
    switch(state) {
        case Opening:
            if(sigmaDelta >= 1) setState(Neutral);
            break;
        case Passing:
            if(sigmaDelta >= 1) {
                if(postDip) callback((Button*)this);
                killDefered();
            }
            break;
        case Hovered:
            if(!isHovered)
                ...
                if(!postDip) callback(this);
                setState(Passing);
            }
            break;
        case Neutral:
            if(isHovered) setState(Hovered);
            break;
    }
    if(isHovered)
...
void Button::postProcessingRender() {
    ...
    const int i = e < .5 ? std::floor(e * text.length()) : std::ceil(e * text.length());
    scratch = text[i];
    text[i] = '\0';

    Vector2 thingPos = position + (Vector2){-textDems.x / 2, -300 - textDems.y};
    DrawRectangleV(
      thingPos - (Vector2){padding + BORDER_WIDTH, padding + BORDER_WIDTH},
      (Vector2){(textDems.x + 2 * (padding + BORDER_WIDTH)) * e,
                textDems.y + 2 * (padding + BORDER_WIDTH)},
      SEPERATORCOLOUR
    );
    DrawRectangleV(
      thingPos - (Vector2){padding, padding},
      (Vector2){(textDems.x + 2 * padding) * e, textDems.y + 2 * padding},
      BACKCOLOUR
    );
