...
void Particle::render() {
    ...
    // draw a square!1!!!!
    DrawRectanglePro(
      (Rectangle){position.x, position.y, length, length},
      (Vector2){length / 2.0f, length / 2.0f},
      rotation,
      colour
    );
}
...
void Particle::process(float delta) {
    ...
    rotation +=
      rotationalVelocity *
      delta;  // btw the * delta is because it's m/s^2 and so you have to account for that s^2 there
    lifetime += delta;
...
Particle::Particle(Vector2 pos, Vector2 vel) : Entity2D("Particle", pos), velocity(vel) {
    ...
    velocity = Vector2Add(
      velocity,
      (Vector2){scalar * ((float)rand() / (float)RAND_MAX) - scalar / 2.0f,
